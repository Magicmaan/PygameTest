...
class ParticleHandler:
    def __init__(self, game):
        Texture.load_texture("particles", (8, 8))
        self.game = game
        self.particles = []
        self.particleCache = {}
        # power controls amount of offset / effect 0 - 1
        self.windPow = Vector2(1, 0)
        # axis will range from -1 to 1
        self.windAxis = Vector2(-0.5, 0)
    ...
    def draw(self, surface):
        ...
        for i, particle in enumerate(self.particles):
            ...
            if (
                pos.x + img.get_width() + self.game.camera.x < 0
                or pos.x + self.game.camera.x > surface.get_width()
            ):
                particle[4] = 0

            if (pos.y + img.get_height() + self.game.camera.y < 0) or (
                pos.y + self.game.camera.y
                > self.game.camera.y + self.game.resolution[1]
            ):
                particle[4] = 0

            # draw if alive, else remove
            if (particle[4] / self.game.TimeMult) > 0:
                surface.blit(img, pos + self.game.camera.position)
                # decrease particle lifespan
                particle[4] -= 1
            else:
                self.particles.pop(i)

    def add(self, position: Vector2, type="default", count=1, addArgs={}):
        ...
        for n in range(count):
            ...
            particleArgs = {**typeArgs(addArgs), **addArgs}
            # random offset for animations
            particleArgs["randomOffset"] = [
            ...
            if particleArgs["randomTex"] == True:
                randomImg = random.randint(
                    particleArgs["randomTexBounds"][0],
                    particleArgs["randomTexBounds"][1],
                )
            ...
            pos.x += random.randint(
                -particleArgs["spread"][0], particleArgs["spread"][0]
            )
            pos.y += random.randint(
                -particleArgs["spread"][1], particleArgs["spread"][1]
            )

            # add to particle list
            self.particles.append(
                [img, pos, type, particleArgs, particleArgs["lifespan"]]
            )

    def pleafAdd(self, addArgs={}):
        addAccel = Vector2(0, 0)
        if "Acceleration" in addArgs.keys():
            addAccel = addArgs["Acceleration"]
        particleArgs = {
            "UseVelocity": True,
            "Acceleration": Vector2(
                random.uniform(-1.5, 1.5) + addAccel.x,
                random.uniform(0, 10) + addAccel.y,
            ),
            "texture": 4,
            "randomTex": True,
            "randomTexBounds": (4, 7),
    ...
    def pdefaultAdd(self, addArgs={}):
        particleArgs = {
            "UseVelocity": True,
            "Acceleration": Vector2(random.uniform(-4, 4), random.uniform(-4, 4)),
            "wind": True,
            "texture": 0,
            "randomTex": True,
            "randomTexBounds": (0, 3),
    ...
    def pleafUpdate(self, particle):
        ...
        particle[1].x += (
            particleCalculations.x * self.game.dt * pArgs["randomOffset"][0]
        )
        particle[1].y += (
            (pArgs["Acceleration"].y * self.game.dt)
            + pArgs["randomOffset"][1]
            + (particleCalculations.y * self.game.dt)
        )
    ...
    def pwindUpdate(self):
        ...
        self.windAxis.x = 1
        self.windPow.x = 100
        self.windOffset = Vector2(self.windPow * self.windAxis)

        if self.windOffset == 0:
            self.windOffset = Vector2(0, 0)
